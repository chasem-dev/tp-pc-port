...
#define PC_GX_MAX_QUAD_INDICES (PC_GX_MAX_VERTS / 4 * 6)
#define PC_GX_MAX_ATTRIB_SIZE 64
#define PC_GX_MAX_ATTR        26
#define PC_GX_MAX_VTXFMT      8
#define PC_GX_MAX_TEV_STAGES  16  /* TP uses up to 16 TEV stages */

/* Texture object stored by GXInitTexObj */
typedef struct {
    void*  image_ptr;
    u16    width, height;
    u32    format;              /* GX_TF_I4..GX_TF_CMPR */
    u32    wrap_s, wrap_t;
    u32    mipmap;
    u32    min_filter, mag_filter;
    f32    min_lod, max_lod, lod_bias;
    u32    max_aniso;
    u32    ci_format;           /* original format for CI textures */
    u32    tlut_name;           /* TLUT slot 0-15 */
    GLuint gl_tex;              /* cached GL texture (0 = not uploaded yet) */
} PCTexObj;
